			...
			if ($tableExists) {
				...
				if (isset($_FILES['pdf_file']) && $_FILES['pdf_file']['error'] === UPLOAD_ERR_OK) {
					...
					if ($ext !== 'pdf') {
						...
						// Use smaller of: 50MB or server limit (but at least 10MB)
						$maxSize = min(50 * 1024 * 1024, $serverMaxSize ?: (50 * 1024 * 1024));
...
<head>
	...
	<style>
		...
		.btn { display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; border: 1px solid #e5e7eb; border-radius: 8px; background: #ffffff; cursor: pointer; text-decoration: none; font-size: 14px; transition: all 0.2s; }
		.btn:hover { background: #f8fafc; }
		.btn-primary { background: #667eea; color: #ffffff; border-color: #667eea; }
		.btn-primary:hover { background: #5568d3; }
		.btn-secondary { background: #6b7280; color: #ffffff; border-color: #6b7280; }
		.btn-secondary:hover { background: #4b5563; }
		.alert { padding: 12px 16px; border-radius: 8px; margin-bottom: 16px; }
		.alert-error { background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; }
		.form-container { background: #ffffff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 24px; max-width: 800px; }
		.form-group { margin-bottom: 20px; }
		.form-group label { display: block; margin-bottom: 8px; font-weight: 500; color: #111827; font-size: 14px; }
...
<form method="POST" enctype="multipart/form-data" class="form-container">
	...
	<div class="form-group">
		...
		<input type="text" id="title" name="title" value="<?php echo escape($_POST['title'] ?? ''); ?>" required class="form-control">
		<small>This will be used as the page title.</small>
	</div>
	
	<div class="form-group">
		<label for="slug">Slug <span class="required">*</span></label>
		<input type="text" id="slug" name="slug" value="<?php echo escape($_POST['slug'] ?? ''); ?>" required class="form-control" pattern="[a-z0-9-]+">
		<small>URL-friendly identifier (lowercase letters, numbers, and hyphens only). Example: product-guide-2024</small>
	</div>
	...
	<div class="form-group">
		...
		<small>Maximum file size: 50MB. Only PDF files are allowed.</small>
	</div>
