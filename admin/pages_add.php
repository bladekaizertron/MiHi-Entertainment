        ...
        <div class="w-72 panel border-r flex flex-col z-10">
            ...
            <div class="p-4 overflow-y-auto flex-1">
                ...
                <div class="component-item" draggable="true" data-type="gallery_grid">
                    <i class="fas fa-th text-yellow-400 w-5"></i> <span>Gallery Image Grid</span>
                </div>
            </div>
        </div>
...
<body class="bg-white" style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <script>
        ...
        function exposeFunctionsToIframe() {
            ...
            // Expose gallery grid functions to iframe window
            iframeWindow.addGalleryGridItem = addGalleryGridItem;
            iframeWindow.removeGalleryGridItem = removeGalleryGridItem;
            iframeWindow.changeGalleryGridImage = changeGalleryGridImage;
            
            // Expose other functions that might be called from templates
            iframeWindow.addFeatureCard = addFeatureCard;
            iframeWindow.removeFeatureCard = removeFeatureCard;
            iframeWindow.addSplitScreenFeaturePoint = addSplitScreenFeaturePoint;
            iframeWindow.removeSplitScreenFeaturePoint = removeSplitScreenFeaturePoint;
            iframeWindow.changeSplitScreenMedia = changeSplitScreenMedia;
            iframeWindow.removeSplitScreenMedia = removeSplitScreenMedia;
            iframeWindow.addVideoCard = addVideoCard;
            iframeWindow.removeVideoCard = removeVideoCard;
            iframeWindow.changeVideoCardMedia = changeVideoCardMedia;
            iframeWindow.openVideoModal = openVideoModal;
            iframeWindow.closeVideoModal = closeVideoModal;
        }
        ...
        function setupIframeListeners(doc) {
            ...
            doc.addEventListener('click', (e) => {
                ...
                // Allow buttons with onclick handlers to execute (e.g., gallery grid buttons)
                const clickedButton = e.target.closest('button[onclick]');
                if (clickedButton) {
                    // Don't prevent default - let the onclick handler execute
                    return;
                }
        ...
        function selectElement(el) {
            ...
            // If selected element is inside a gallery grid section, show Gallery Grid inspector
            let galleryGridSection = null;
            if (el.querySelector('#gallery-grid-container')) {
                galleryGridSection = el;
                typeLabel = "Gallery Image Grid";
            } else if (el.closest('[data-editable]') && el.closest('[data-editable]').querySelector('#gallery-grid-container')) {
                galleryGridSection = el.closest('[data-editable]');
                typeLabel = "Gallery Image Grid";
            }
            ...
            // Check if it's a gallery grid section
            const isGalleryGrid = !!galleryGridSection;
            ...
            inspector.innerHTML = `
                ...
                ${isGalleryGrid && galleryGridSection ? (() => {
                    const container = galleryGridSection.querySelector('#gallery-grid-container');
                    const galleryItems = container ? Array.from(container.querySelectorAll('.gallery-item')) : [];
                    const escapeHtml = (s) => (s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                    const itemListHtml = galleryItems.length ? galleryItems.map((item, i) => {
                        const img = item.querySelector('img');
                        const src = img ? img.src : '';
                        const title = item.querySelector('.gallery-item-title');
                        const titleText = title ? (title.textContent || 'Item ' + (i + 1)) : 'Item ' + (i + 1);
                        const size = item.getAttribute('data-size') || 'medium';
                        const label = escapeHtml(titleText.substring(0, 20) + (titleText.length > 20 ? '…' : ''));
                        return `<div class="flex items-center justify-between gap-2 py-1.5 px-2 rounded bg-zinc-800/50 border border-zinc-700/50">
                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                ${src ? `<img src="${escapeHtml(src)}" alt="${escapeHtml(titleText)}" class="w-8 h-8 object-cover rounded flex-shrink-0">` : `<div class="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-500 rounded flex-shrink-0"></div>`}
                                <div class="flex-1 min-w-0">
                                    <span class="text-xs text-zinc-300 truncate block" title="${escapeHtml(titleText)}">${label}</span>
                                    <span class="text-[10px] text-zinc-500">${size}</span>
                                </div>
                            </div>
                            <div class="flex gap-1 flex-shrink-0">
                                <button type="button" onclick="changeGalleryGridImageByIndex(${i})" class="text-blue-400 hover:text-blue-300 p-1 rounded hover:bg-blue-500/10" title="Change image">
                                    <i class="fas fa-image text-xs"></i>
                                </button>
                                <button type="button" onclick="removeGalleryGridItemByIndex(${i})" class="text-red-500 hover:text-red-400 p-1 rounded hover:bg-red-500/10" title="Remove item">
                                    <i class="fas fa-times text-xs"></i>
                                </button>
                            </div>
                        </div>`;
                    }).join('') : '<p class="text-xs text-zinc-500 py-2">No items yet. Add one below.</p>';
                    
                    return `
                <div class="mb-4 border-t border-zinc-700 pt-4">
                    <label class="block text-xs text-zinc-500 mb-2">Gallery Items (${galleryItems.length})</label>
                    <div class="space-y-1.5 max-h-48 overflow-y-auto mb-2">${itemListHtml}</div>
                    <button onclick="addGalleryGridItemFromInspector()" class="w-full bg-zinc-700 hover:bg-zinc-600 text-white px-3 py-2 rounded text-xs transition-colors flex items-center justify-center gap-2">
                        <i class="fas fa-plus"></i>
                        <span>Add Gallery Item</span>
                    </button>
                    <div class="text-xs text-zinc-500 mt-2">Choose size: small, medium, or large</div>
                </div>
                `;
                })() : ''}
        ...
        // --- Gallery Image Grid Functions ---
        function addGalleryGridItemFromInspector() {
            // Call the main function with null to indicate it's from inspector
            addGalleryGridItem(null);
        }
        
        function changeGalleryGridImageByIndex(index) {
            const doc = iframe.contentDocument;
            if (!doc) return;
            
            const container = doc.querySelector('#gallery-grid-container');
            if (!container) return;
            
            const items = container.querySelectorAll('.gallery-item');
            if (index >= 0 && index < items.length) {
                changeGalleryGridImage(items[index]);
            }
        }
        
        function removeGalleryGridItemByIndex(index) {
            const doc = iframe.contentDocument;
            if (!doc) return;
            ...
            const items = container.querySelectorAll('.gallery-item');
            if (index >= 0 && index < items.length) {
                removeGalleryGridItem(items[index]);
            }
        }
        
        function addGalleryGridItem(button) {
            const doc = iframe.contentDocument;
            if (!doc) return;
            ...
            if (button && button.closest) {
                ...
                container = button.closest('section').querySelector('#gallery-grid-container');
            } else {
                // Called from parent, find in selected element or search
                if (selectedElement) {
                    container = selectedElement.querySelector('#gallery-grid-container') || 
                               selectedElement.closest('section')?.querySelector('#gallery-grid-container');
                }
                if (!container) {
                    const sections = doc.querySelectorAll('[data-editable]');
                    sections.forEach(sec => {
                        if (sec.querySelector('#gallery-grid-container')) container = sec.querySelector('#gallery-grid-container');
                    });
                }
            }
            
            if (!container) {
                alert('Could not find gallery grid container. Please make sure a Gallery Image Grid section is added to the page.');
                return;
            }
            
            // Ask for size
            const size = prompt('Choose item size (small/medium/large):', 'medium');
            if (!size || !['small', 'medium', 'large'].includes(size.toLowerCase())) {
                alert('Invalid size. Please choose small, medium, or large.');
                return;
            }
            
            // Random gradient colors for placeholder
            const gradients = [
            ...
            const newItem = doc.createElement('div');
            newItem.className = `${size.toLowerCase()} gallery-item`;
            newItem.setAttribute('data-size', size.toLowerCase());
            newItem.innerHTML = `
                <img src="" alt="Gallery image" style="display: none;">
                <div style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; background: ${randomGradient}; color: white; font-size: 1.5rem;">Image Placeholder</div>
                <div contenteditable="true" class="gallery-item-title outline-none">GALLERY TITLE</div>
            `;
            
            container.appendChild(newItem);
            
            showNotification('success', 'Item Added', `${size.charAt(0).toUpperCase() + size.slice(1)} gallery item added successfully.`);
        }
        
        function removeGalleryGridItem(button) {
            if (confirm('Are you sure you want to remove this gallery item?')) {
                const item = button.closest('.gallery-item');
                if (item) {
                    item.remove();
                    showNotification('success', 'Item Removed', 'Gallery item removed successfully.');
                }
            }
        }
        
        function changeGalleryGridImage(button) {
            const doc = iframe.contentDocument;
            if (!doc) return;
            ...
            window.currentGalleryGridItem = item;
            
            // Open media modal
            openMediaModal('image');
        }
        
        // Override handleMediaUpload to support gallery grid
        const originalHandleMediaUpload = window.handleMediaUpload;
        window.handleMediaUpload = function() {
            // Check if we're updating a gallery grid item
            if (window.currentGalleryGridItem) {
                const fileInput = document.getElementById('mediaFileInput');
                const file = fileInput.files[0];
                ...
                formData.append('csrf_token', csrfToken);
                formData.append('action', 'upload_video'); // Reuse the same endpoint
                
                showNotification('info', 'Uploading...', 'Please wait while your image is being uploaded.');
                
                fetch('?action=upload_video', {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        const doc = iframe.contentDocument;
                        const img = window.currentGalleryGridItem.querySelector('img');
                        const placeholder = window.currentGalleryGridItem.querySelector('div[style*="Image Placeholder"]');
                        ...
                        closeMediaModal();
                        window.currentGalleryGridItem = null;
                        showNotification('success', 'Upload Complete', 'Image uploaded and added to gallery successfully.');
                    } else {
            ...
            if (originalHandleMediaUpload) {
                originalHandleMediaUpload();
            }
        };
        
        // Override handleMediaUrl to support gallery grid
        const originalHandleMediaUrl = window.handleMediaUrl;
        window.handleMediaUrl = function() {
            // Check if we're updating a gallery grid item
            if (window.currentGalleryGridItem) {
                const urlInput = document.getElementById('mediaUrlInput');
                const url = urlInput.value.trim();
                ...
                closeMediaModal();
                window.currentGalleryGridItem = null;
                showNotification('success', 'Image Added', 'Image added to gallery successfully.');
                
                return;
            }
            
            // Otherwise, use original function
            if (originalHandleMediaUrl) {
                originalHandleMediaUrl();
            }
        };
        
        // --- Expose Functions to Iframe (MUST be called after all functions are defined) ---
        // This needs to be called AFTER the iframe is loaded AND after all functions are defined
        // We'll call it when the page is fully loaded
